...
class Wechat {
	...
	const TEMPLATE_LIST 		  = 'https://api.weixin.qq.com/cgi-bin/template/get_all_private_template';
	const TEMPLATE_DELETE		  = 'https://api.weixin.qq.com/cgi-bin/template/del_private_template';
	/* JSAPI_TICKET获取地址 */
	...
	public function menu_create($menus = array()) {
		...
		//创建菜单之前，执行删除操作
		//$this->menu_delete();
		$menuJson = json_encode($menus);//$this->json_encode($menus);
		$params = urldecode($menuJson);
	...
	/**
	 * 获取已添加至帐号下所有模板列表
	 * @return mix  boolean|array
	 * @author lijin
	 */
	public function getTemplateList()
	{
		$url     = self::TEMPLATE_LIST . '?access_token=' . $this->access_token;
		$jsonStr = $this->http($url, array(), 'get');
		$jsonArr = $this->parseJson($jsonStr);
		if ($jsonArr)
		{
			return $jsonArr;
	...
	/**
	 * 删除帐号下的模板
	 * @param  array  $param ["template_id" => "Dyvp3-Ff0cnail_CDSzk1fIc6-9lOkxsQE7exTJbwUE"]
	 * @return boolean
	 * @author lijin
	 */
	public function deleteTemplate($param)
	{
		...
		$url     = self::TEMPLATE_DELETE . '?access_token=' . $this->access_token;
		$jsonStr = $this->http($url, $params, 'post');
		$jsonArr = $this->parseJson($jsonStr);
