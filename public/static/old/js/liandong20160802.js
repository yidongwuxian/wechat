...
/***************************方法*****************************/
	...
	function getData(nextIndex,val,$next){console.log(nextIndex)
	...
	function ajaxCallBack(msg,nextIndex,$next){
        ...
        if($next.attr('id') == 'pg_city'){
            ...
            setTimeout(function(){
                ...
                setSubmitData('city',$('#pg_city').val(),$('#pg_city').text());
            },0)
        }
	...
	function createAutoComplete(key,list){
		...
		that.autoComplete({
            ...
            //btmBtnText:'切换到地址搜索',
			ontxtinput:function(keyword){console.log($('#pg_city').find("option:selected").text())
				if(!keyword){
					setSubmitData(key,'','');
