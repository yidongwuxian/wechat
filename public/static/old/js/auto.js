...
;(function(window,$){
    ...
    AutoComplete.prototype = {
        ...
        createLayer : function(){
            ...
            return $('<div id="auto_layer_'+ _id +'" class="auto_layer '+this.opts.myClass+'" style="display:none;top:'+this.top+'px;height:'+this.height+'"></div>')
                .appendTo(this.elem.parent().addClass('auto_wrap'));//+'px;width:'+this.width+left:'+this.left+'px;
        },
        init : function(){
            var that = this;
            var _id = that.elem.attr('id');
            ...
            that.elem.on('input propertychange focus',function(){
                if (!$.trim($(this).val())) {
        ...
        localSearch : function(list,keyword){
            ...
            createResultList(that.$layer,result);
        },
        //ajax请求查询结果
        ajaxSearch : function(keyword){
            ...
            $.ajax({
                ...
                var _list = data.data;console.log(_list)
                if(_list && _list instanceof Array && _list.length){	//数据为数组
                    createResultList(that.$layer,_list);
                }else{
    ...
    //创建搜索结果的列表
    function createResultList($con,data){
        var _html = '<ul>';
        data.forEach(function(item,i){
            _html += '<li data-id="'+item.id+'">'+item.thirdOrgName+'</li>';
        })
        _html += '</ul>';
        $con.html(_html);
    }
