...
/***************************方法*****************************/
	...
	function getData(nextIndex,val,$next){
		...
		if(nextIndex<4){
			param[key] = val;
            if(nextIndex==0){
                $('#auto_layer_pg_village').hide();
                //param['province'] = $('#pg_province').find('option:selected').val();//(!可选择城市修改此处注释，不可选择是不注释)
            }else if(nextIndex<3){
                $('#auto_layer_pg_village').hide();
                param['province'] = $('#pg_province').find('option:selected').val();//(!可选择城市修改此处不注释，下面注释，不可选择相反)
	...
	function ajaxCallBack(msg,nextIndex,$next){
        ...
        if($next.attr('id') == 'pg_city'){
            ...
            //$next = $('#pg_village');
            setTimeout(function(){
                ajaxCallBack({success:true,data:{}},2,$('#pg_village'));//当默认城市出来时，模拟请求一下，让next值校正(!可选择城市修改此处2，不可选择是1)
